...
const char Keyboard::keymap[0x80][8] =
{
    ...
    /*48*/{30, 0x0, 0x0}, {0x0, 0x0, 0x0}, {'-', '-', '-'}, {28, 0x0, 0x0},
    /*4C*/{0x0, 0x0, 0x0}, {29, 0x0, 0x0}, {'+', '+', '+'}, {0x0, 0x0, 0x0},
    /*50*/{31, 0x0, 0x0}, {0x0, 0x0, 0x0}, {'0', 0x0, 0x0}, {'.', '.', '.'},
...
Error Keyboard::read(s8 *buffer, Size size, Size offset)
{
	...
	else if (modifierKey == NO_MODIFIER_KEY && keycode == 0xA8) {
		modifierKey = DIACRITIC_KEY;
	}
	else if (modifierKey == SHIFT_KEY && keycode == 0xA8) {
		modifierKey = DIACRITIC_SHIFT_KEY;
	}
	else if (modifierKey <= ALT_GR_SHIFT_KEY && keycode == 0xE0) {
