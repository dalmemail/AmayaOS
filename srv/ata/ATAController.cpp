...
Error ATAController::initialize()
{
    ...
    switch (inb(ATA_BASE_CMD0 + ATA_REG_STATUS)) {
        case 0:
            syslog(LOG_INFO, "No se ha detectado ningun dispositivo ATA");
            break;
        ...
        default:
            ...
            syslog(LOG_INFO,"Dispositivo ATA detectado: SERIAL=%20s FIRMWARE=%8s "
...
{
    syslog(LOG_INFO, "Se ha interrumpido ATA en la IRQ %u", vector);
    return ESUCCESS;
}
