...
/**
 * Compare two memory sector, by only a maximum number of bytes.
 * @param dest Destination memory to compare.
 * @param src Source memory to compare.
 * @param count Number of bytes to compare.
 * @return Zero if equal, non-zero if not.
 */
extern C int memcmp(const void *dest, const void *src, size_t count);

/**
 * @brief Memory scanning operation.
 *
 * The memchr() function shall locate the last occurrence of c
 * (converted to a char) in the string pointed to by str. The terminating
 * NUL character is considered to be part of the memory.
 *
 * @param str Memory sector to search in.
 * @param c Character to look for.
 * @param n Number of bytes to scan.
 * @return Upon successful completion, memrchr() shall return a pointer
 *         to the byte or a null pointer if c does not occur in the string.
 */
extern C void *memchr(const void *str, int c, size_t n);
